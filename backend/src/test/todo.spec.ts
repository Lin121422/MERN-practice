...
describe('Form test', () => {
    ...
    it('should successfully post a Todo to mongodb and can be found', async () => {
        const response = await server.inject({  // 建立response
            method: 'POST',
            url: '/api/todos',  // 要前綴/api，server.ts有寫
            payload: {
                name: 'clean my desk',
        ...
        expect(response.statusCode).toBe(201)  // expect toBe 就是期望這個東西是甚麼，類似 if a == b概念
        const res: { todo: ITodo } = JSON.parse(response.body)  // 解析 body，拆成類似上面response樣子
        console.log(`post Todo: ${response.body}`)  // 寫log
        ...
        expect(getResponse.statusCode).toBe(200)  // 應該要有 GET OK
